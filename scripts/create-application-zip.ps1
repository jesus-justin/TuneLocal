...
$stagingRoot = Join-Path $repoRoot "release\_staging_app"
$packageRoot = Join-Path $stagingRoot "TuneLocal-Application-$Version"
$appFolder = Join-Path $packageRoot "TuneLocal"
$engineFolder = Join-Path $packageRoot "Engine"
$zipName = "TuneLocal-Application-$Version.zip"
...
"@
Set-Content -Path (Join-Path $packageRoot "TuneLocalApp.bat") -Value $rootBat -Encoding Ascii
...
public static class Program
{
    ...
    public static void Main()
    {
        ...
        {
            MessageBox.Show(
                "XAMPP not found at C:\\xampp. Install XAMPP first from https://www.apachefriends.org/",
                "TuneLocal Application",
        ...
        {
            MessageBox.Show(
                "Could not copy app files to htdocs: " + ex.Message,
                "TuneLocal Application",
                MessageBoxButtons.OK,
                MessageBoxIcon.Error
            );
...
$exePath = Join-Path $packageRoot "TuneLocalApp.exe"
$compiled = $false
...
if (-not $compiled) {
    Write-Warning "Could not compile TuneLocalApp.exe launcher. Fallback TuneLocalApp.bat is available."
}
...
"@
Set-Content -Path (Join-Path $packageRoot "README-APPLICATION.txt") -Value $readme -Encoding Ascii
...
Write-Host "Created application ZIP: $zipPath"
